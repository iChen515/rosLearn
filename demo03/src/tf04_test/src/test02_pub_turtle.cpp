...
void doPose(const turtlesim::Pose::ConstPtr & pose){
    ...
    //  a 创建发布对象
    static tf2_ros::TransformBroadcaster pub;   // 设置为静态变量，防止每一次调用回调函数都重新生成pub
    //  b 组织被发布的数据
    ...
    // 坐标系四元数设置
    /*
        位姿信息中没有四元数，但有个偏行角度，又已知乌龟是 2D，
        没有翻滚与俯仰角度，所以可以得出乌龟的欧拉角：0 0 theta
    */
    tf2::Quaternion qtn;
...
int main(int argc, char *argv[]) {
    ...
    /*
        解析 launch 文件，通过args传入参数
    */
   if (argc!= 2){
        ROS_INFO("argc %d",argc);
        ROS_ERROR("参数错误，请传入一个参数");
        return 1;
    } else {
        turtle_name = argv[1];
    }
